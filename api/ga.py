    ...
    for m in range(pro_time_matrix.shape[1]):
        for w in range(pro_time_matrix.shape[2]):
            if pro_time_matrix[random_index, m, w] != -1:
                avail_machines.append(m)
                break
    
    random_avail_machine = np.random.choice(avail_machines)

    for w in range(pro_time_matrix.shape[2]):
        if pro_time_matrix[random_index, random_avail_machine, w] != -1:
            avail_workers.append(w)
...
def wa_mutation(ma, wa, pro_time_matrix):
    ...
    for w in range(pro_time_matrix.shape[2]):
        if pro_time_matrix[random_index, machine, w] != -1:
            avail_workers.append(w)
...
def pox_crossover(p1, p2, num_commands):
    job_list = [i + 1 for i in range(num_commands)]

    if num_commands > 1:
        random_devide_job_index = np.random.randint(1, num_commands)
    else: 
        random_devide_job_index = 1
